...
export default function ItemsEditor({ account, initialItems, onSave, onClose }: ItemsModalProps) {
  ...
  const renderFields = (item: any, index: number) => {
    ...
    if (category === 'Time Deposits') {
      fields = (
        <>
          <div className="col-span-1 sm:col-span-2"><label className="block text-xs text-gray-500 mb-1">{t('assets.platform') || 'Platform'}</label><input type="text" className={inputClass} value={item.platform || ''} onChange={e => handleChange(index, 'platform', e.target.value)} /></div>
          ...
          <div className="col-span-1 sm:col-span-2"><label className="block text-xs text-gray-500 mb-1">{t('common.amount') || 'Amount'}</label><input type="number" className={inputClass} value={item.amount || ''} onChange={e => handleChange(index, 'amount', e.target.value)} /></div>
        </>
      );
    } else if (category === 'Investment Insurance') {
      fields = (
        <>
          <div className="col-span-1 sm:col-span-2"><label className="block text-xs text-gray-500 mb-1">{t('insurance.company') || 'Company'}</label><input type="text" className={inputClass} value={item.company || ''} onChange={e => handleChange(index, 'company', e.target.value)} /></div>
          <div className="col-span-1 sm:col-span-2"><label className="block text-xs text-gray-500 mb-1">{t('insurance.productName') || 'Product Name'}</label><input type="text" className={inputClass} value={item.product_name || ''} onChange={e => handleChange(index, 'product_name', e.target.value)} /></div>
          <div className="col-span-1 sm:col-span-2"><label className="block text-xs text-gray-500 mb-1">{t('insurance.productType') || 'Product Type'}</label><input type="text" className={inputClass} value={item.product_type || ''} onChange={e => handleChange(index, 'product_type', e.target.value)} /></div>
          <div className="col-span-1 sm:col-span-2"><label className="block text-xs text-gray-500 mb-1">{t('common.amount') || 'Amount'}</label><input type="number" className={inputClass} value={item.amount || ''} onChange={e => handleChange(index, 'amount', e.target.value)} /></div>
        </>
      );
      ...
      fields = (
        <>
          <div className="col-span-1 sm:col-span-2"><label className="block text-xs text-gray-500 mb-1">{t('assets.platform') || 'Platform'}</label><input type="text" className={inputClass} value={item.platform || ''} onChange={e => handleChange(index, 'platform', e.target.value)} /></div>
          <div className="col-span-1 sm:col-span-2"><label className="block text-xs text-gray-500 mb-1">{t('assets.productName') || 'Product Name'}</label><input type="text" className={inputClass} value={item.product_name || ''} onChange={e => handleChange(index, 'product_name', e.target.value)} /></div>
          ...
          <div className="col-span-1"><label className="block text-xs text-gray-500 mb-1">{t('common.currency') || 'Currency'}</label><input type="text" className={inputClass} value={item.currency || ''} onChange={e => handleChange(index, 'currency', e.target.value)} /></div>
          <div className="col-span-1 sm:col-span-2"><label className="block text-xs text-gray-500 mb-1">{t('assets.currentValue') || 'Current Value'}</label><input type="number" className={inputClass} value={item.current_value || ''} onChange={e => handleChange(index, 'current_value', e.target.value)} /></div>
        </>
      );
    } else if (category === 'Stocks') {
      fields = (
        <>
          <div className="col-span-1 sm:col-span-2"><label className="block text-xs text-gray-500 mb-1">{t('assets.broker') || 'Broker'}</label><input type="text" className={inputClass} value={item.broker || ''} onChange={e => handleChange(index, 'broker', e.target.value)} /></div>
          <div className="col-span-1 sm:col-span-2"><label className="block text-xs text-gray-500 mb-1">{t('assets.stockCode') || 'Stock Code'}</label><input type="text" className={inputClass} value={item.stock_code || ''} onChange={e => handleChange(index, 'stock_code', e.target.value)} /></div>
      ...
      fields = (
        <>
          <div className="col-span-1 sm:col-span-2"><label className="block text-xs text-gray-500 mb-1">{t('assets.lender') || 'Lender'}</label><input type="text" className={inputClass} value={item.lender || ''} onChange={e => handleChange(index, 'lender', e.target.value)} /></div>
          <div className="col-span-1"><label className="block text-xs text-gray-500 mb-1">{t('assets.startDate') || 'Start Date'}</label><input type="date" className={inputClass} value={item.start_date || ''} onChange={e => handleChange(index, 'start_date', e.target.value)} /></div>
          ...
          <div className="col-span-1"><label className="block text-xs text-gray-500 mb-1">{t('assets.interestRate') || 'Interest Rate'}</label><input type="text" className={inputClass} value={item.interest_rate || ''} onChange={e => handleChange(index, 'interest_rate', e.target.value)} /></div>
          <div className="col-span-1"><label className="block text-xs text-gray-500 mb-1">{t('common.currency') || 'Currency'}</label><input type="text" className={inputClass} value={item.currency || ''} onChange={e => handleChange(index, 'currency', e.target.value)} /></div>
          <div className="col-span-1 sm:col-span-2"><label className="block text-xs text-gray-500 mb-1">{t('assets.totalAmount') || 'Total Amount'}</label><input type="number" className={inputClass} value={item.total_amount || ''} onChange={e => handleChange(index, 'total_amount', e.target.value)} /></div>
          ...
          <div className="col-span-1 sm:col-span-2 lg:col-span-4 mt-2 mb-1 border-t pt-2"><span className="text-xs font-semibold text-gray-700">{t('record.inventoryDetails') || 'Inventory Details'}</span></div>
          ...
          <div className="col-span-1 sm:col-span-2"><label className="block text-xs text-gray-500 mb-1">{t('record.monthlyRepayment') || 'Monthly Repayment'}</label><input type="number" className={cn(inputClass, "bg-gray-50")} value={(parseFloat(item.repayment_principal || '0') + parseFloat(item.repayment_interest || '0')) || ''} readOnly /></div>
        </>
      );
    } else if (category === 'Receivables') {
      fields = (
        <>
          <div className="col-span-1 sm:col-span-2"><label className="block text-xs text-gray-500 mb-1">{t('assets.borrower') || 'Borrower'}</label><input type="text" className={inputClass} value={item.borrower || ''} onChange={e => handleChange(index, 'borrower', e.target.value)} /></div>
          <div className="col-span-1"><label className="block text-xs text-gray-500 mb-1">{t('common.currency') || 'Currency'}</label><input type="text" className={inputClass} value={item.currency || ''} onChange={e => handleChange(index, 'currency', e.target.value)} /></div>
          ...
          <div className="col-span-1"><label className="block text-xs text-gray-500 mb-1">{t('assets.interestRate') || 'Interest Rate'}</label><input type="text" className={inputClass} value={item.interest_rate || ''} onChange={e => handleChange(index, 'interest_rate', e.target.value)} /></div>
          <div className="col-span-1"><label className="block text-xs text-gray-500 mb-1">{t('assets.startDate') || 'Start Date'}</label><input type="date" className={inputClass} value={item.start_date || ''} onChange={e => handleChange(index, 'start_date', e.target.value)} /></div>
          <div className="col-span-1 sm:col-span-2"><label className="block text-xs text-gray-500 mb-1">{t('assets.endDate') || 'End Date'}</label><input type="date" className={inputClass} value={item.end_date || ''} onChange={e => handleChange(index, 'end_date', e.target.value)} /></div>
        </>
      );
    }
    ...
    return (
      <div key={item.id || index} className="p-4 bg-gray-50 rounded-xl border border-gray-100 relative group">
        <button 
          ...
          className="absolute top-2 right-2 p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg sm:opacity-0 sm:group-hover:opacity-100 transition-opacity"
        >
          <Trash2 size={16} />
        </button>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:pr-8">
          {fields}
        </div>
      </div>
    );
